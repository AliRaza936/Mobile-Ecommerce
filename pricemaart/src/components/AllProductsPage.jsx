...
const AllProductPage = ({product ,loading:initialLoading}) => {
...
const [productsPerPage, setProductsPerPage] = useState(20);
    ...
    const handleRAMChange = (ram) => {
      ...
      setTimeout(() => {
          ...
          setSelectedRAM((prevRAM) =>
            prevRAM.includes(ram) ? prevRAM.filter((r) => r !== ram) : [...prevRAM, ram]
        );
      }, 1000);
    ...
    const filteredProducts = product.filter((item) => {
        // RAM filter (only if item has productRAM)
        if (selectedRAM.length > 0) {
            const productRAM = item.productRAM;
            if (!productRAM || !selectedRAM.includes(productRAM)) {
                return false;
            }
        }
        // Storage filter (only if item has phoneSpecs)
        if (selectedStorage.length > 0) {
            const productStorage = item.phoneSpecs?.storage?.toLowerCase().replace("gb", "").trim();
      ...
      setIsOpen(false);
        ...
        setTimeout(() => {
            ...
            setSelectedStorage((prevStorage) =>
              prevStorage.includes(storage)
                  ? prevStorage.filter((s) => s !== storage)
                  : [...prevStorage, storage]
          );
        }, 1000);   
...
>
  ...
  <div className="md:mb-1">
    <h3 className="font-medium text-sm sm:text-base ">RAM</h3>
    {["4gb", "6gb", "8gb", "12gb"].map((ram) => (
      <label key={ram} className="block  text-sm  xl:text-base">
        <input
          type="checkbox"
  ...
  <div className="md:mb-1 mb-2">
    <h3 className="font-medium text-sm sm:text-base mb-1">Storage</h3>
    {["64", "128", "256", "512"].map((storage) => (
      <label key={storage} className="block text-sm xl:text-base">
        <input
          type="checkbox"
