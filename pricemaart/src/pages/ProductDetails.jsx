...
import { getSingleProd } from "../store/feature/product/productSlice";
...
let loading = useSelector((state)=>state.product?.loading)
 ...
 let [inputQuantity,setInputQuantity] = useState()
  ...
  let hanldeAddToCart = ()=>{
    dispatch(addToCart({id,name,price,pictureUrl,color,discount,quantity:inputQuantity,modelType}))
...
let relatedProduct = async (CatProductId,id)=>{
  ...
  try {
    let result = await axios.get(`${BASE_URL}/products/all?catName=${CatProductId}`,{
  ...
  useEffect(() => {
    if (product?.product?.catName) {
        setCatProductId(product.product.catName);
...
useEffect(()=>{
  ...
  return (
    ...
    <div className="flex justify-between xs:pr-2 xl:pr-7">
...
<div className="flex xs:block gap-10  ">
        ...
        <div className='flex gap-3 mt-2'>
      {
       ...
       <br /><hr />
        <div className="flex justify-center flex-col mt-4 items-center">
            ...
            <p className="text-[14px] xs:w-[95%] xs:pr-3">{product?.product?.description}</p>
  ...
  <table className="w-full border-collapse">
    <thead>
      <tr>
        <th rowSpan={2} className="border border-gray-300 p-3 text-left   xs:text-[12px]   py-5 text-[14px]">STORAGE:</th>
        <th className="border border-gray-300 p-3 text-left font-bold  xs:text-[12px]   py-5 text-[14px]">RAM</th>
        <th className="border border-gray-300 p-3 text-left font-bold   xs:text-[12px]  py-5 text-[14px]">STORAGE</th>
      </tr>
      <tr>
        
        <td className="border border-gray-300 p-3 uppercase py-5  xs:text-[12px]   font-medium text-[14px]">  {product?.product?.productRAM || "N/A"}</td>
        <td className="border border-gray-300 p-3 uppercase  xs:text-[12px]   font-medium text-[14px]"> {product?.product?.phoneSpecs?.storage?.toLowerCase().slice(-1) !== 'b' 
    ? product?.product?.phoneSpecs?.storage + 'GB' 
    : product?.product?.phoneSpecs?.storage}</td>
      </tr>
    </thead>
    <tbody>
      <tr className="">
        <th className="border border-gray-300 p-3 py-5  xs:text-[12px]   text-left text-[14px] text-nowrap">PROCESSOR :</th>
        <td colSpan="2" className="border border-gray-300  xs:text-[12px]   p-3 font-medium text-[14px]">
            {product?.product?.phoneSpecs?.processor}
        </td>
      </tr>
      <tr>
        <th className="border border-gray-300 p-3 text-left  xs:text-[12px]   py-5 text-[14px]">CAMERA :</th>
        <td colSpan="2" className="border border-gray-300  xs:text-[12px]   p-3 font-medium text-[14px]"> {product?.product?.phoneSpecs?.camera}</td>
      </tr>
     
      <tr>
        <th className="border border-gray-300 p-3 text-left  xs:text-[12px]   py-5 text-[14px]">BATTERY :</th>
        <td colSpan="2" className="border border-gray-300 p-3 font-medium text-[14px]" > {product?.product?.phoneSpecs?.batteryCapacity}</td>
      </tr>
      <tr>
        <th className="border border-gray-300 p-3 text-left  xs:text-[12px]   py-5 text-[14px]">DISPLAY :</th>
        <td colSpan="2" className="border border-gray-300  xs:text-[12px]   p-3 font-medium text-[14px]"> {product?.product?.phoneSpecs?.displaySize}</td>
      </tr>
    </tbody>
  </table>
</div>
  ...
  
{
  ...
  <div className="">
            ...
            <div  className=" w-[100px] lg:w-[80px] xl:w-[100px]">
    ...
    2xl:grid-cols-6">
            ...
            <div  className="  xs:w-[70px] w-[100px] lg:w-[80px] xl:w-[140px]">
               ...
               <div className="mt-1">
                ...
                <span className="text-gray-500 text-sm line-through text-nowrap xs:text-[12px] font-semibold">RS:{format()(product?.oldprice)}</span><p className="text-sm text-red-500 xs:text-[12px] font-semibold text-nowrap">RS: {format()(product?.price)}</p></div>
               </div>
            </div>
                 </div>
                  )
