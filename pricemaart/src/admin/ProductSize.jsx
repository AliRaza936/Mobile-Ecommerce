...
const ProductSize = () => {
  ...
  let allSizes = async ()=>{
    try {
        let result = await axios.get(`http://localhost:8000/product/productSize/all`,{
...
let handleDelete = async(id)=>{
    ...
    try {
        let result = await axios.delete(`http://localhost:8000/product/productSize/delete/${id}`,{
            withCredentials:true,
            headers:{
                "Content-Type": "application/json",
            }
...
let getSingleSize = async (id) => {
    try {
        let result = await axios.get(
            `http://localhost:8000/product/productSize/single/${id}`,
...
let handleUpdate = async (id) => {
    ...
    try {
        let result = await axios.put(
            `http://localhost:8000/product/productSize/update/${id}`,
            inputField,
            {
                withCredentials: true,
                headers: {
    ...
    let handleSubmit = async (e) => {
      e.preventDefault();
      if(inputField.name === ''){
        setOpen({open:true,message:'please add product size',severity:'error'})
        return
    ...
    handleUpdate(updateId)
      ...
      try {
        let result = await axios.post(
          "http://localhost:8000/product/productSize/create",
          inputField,
          {
            withCredentials: true,
            headers: {
  ...
  return (
    ...
    <div className="w-full flex justify-between p-5 rounded-lg bg-white">
      <div>
        <h3 className="text-xl font-semibold">Add Product Size</h3>
      </div>
      <div role="presentation" className="">
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <StyledBreadcrumb sx={{ cursor: "default" }} label="Add Product Size" />
        </Breadcrumbs>
      </div>
    </div>
        ...
        <tbody className="relative">
            {
                ...
                allSize?.sizes?.length === 0 ? <div className=" absolute left-[50%] translate-x-[-50%] ">
                 ...
                 (
                     ...
                     <td className="px-6   ">
                      <div className="flex">
                       ...
                       <button onClick={()=>getSingleSize(size._id)}  className="bg-green-600 p-2 rounded-lg scale-90"><MdEdit className="text-[16px] text-white"/></button>
                       <button onClick={()=>handleDelete(size._id)} className="bg-red-500 p-2 rounded-lg scale-90"><MdDelete className="text-[16px] text-white"/>
