...
let allBanner = async ()=>{
 try {
     let result = await axios.get(`${BASE_URL}/saleBanner/all`,{
...
context.setProgress(30)
 ...
 try {
     let result = await axios.delete(`${BASE_URL}/saleBanner/delete/${id}`,{
         withCredentials:true,
         headers:{
             "Content-Type": "multipart/form-data",
         }
...
const handleClose = (event, reason) => {
 ...
 useEffect(()=>{
  ...
  <div>
    <h3 className="text-xl xs:text-lg font-semibold">Sale Banner List</h3>
  </div>
  ...
  <div role="presentation" className="flex flex-wrap gap-2 items-center">
    ...
    <Link to={'/admin/sale-banner-upload'}>
      <button className="bg-green-500 px-3 py-2 font-semibold text-white rounded-md hover:bg-green-600 text-sm sm:text-base">
        Add Banner
      </button>
    </Link>
  </div>
        ...
        <tbody className="">
            ...
            <td className="px-6   ">
             <div className="flex">
              ...
              <button onClick={()=>{navigate(`/admin/update-sale-banner/${banner._id}`)}} className="bg-green-600 p-2 rounded-lg scale-90"><MdEdit className="text-[16px] text-white"/></button>
