...
let deleteCategory = async(id)=>{
  ...
  context.setProgress(30)
    ...
    try {
        let result = await axios.delete(`${BASE_URL}/accessory/delete/${id}`,{
            withCredentials:true,
            headers:{
                "Content-Type": "multipart/form-data",
            }
...
const handleClose = (event, reason) => {
  ...
  <div>
    <h3 className="text-base sm:text-xl font-semibold">Accessory List</h3>
  </div>
  <div role="presentation" className="flex gap-2 items-center">
    ...
    <Link to="/admin/add-accessory">
      <button className="bg-green-500 px-3 xs:text-[13px] font-semibold text-white rounded-md p-2 hover:bg-green-600">
        Add Accessory
      </button>
    </Link>
  </div>
    ...
    <tbody className="">
        ...
        <td className="px-6   ">
         <div className="flex">
          ...
          <button onClick={()=>{navigate(`/admin/update-accessory/${category._id}`)}} className="bg-green-600 p-2 rounded-lg scale-90"><MdEdit className="text-[16px] text-white"/></button>
